...
| `perception` | 观察通道：白名单群的只读观察（默认关闭） |
...
白名单群内"只看不说"的观察与话题跟进，默认零模型调用、仅在明确的跟进判断点消耗一次调用：

```json
"perception": {
    "enabled": true,
    "observe_groups": [8823, 9001]
}
```

它做什么：
- **内容短时缓冲**：白名单群的群聊消息（含 Klein 自己的回复）保留**每群最近 300 条 / 24 小时**（双上限，先到先删），供被 @ 时按相关性挑选上下文——群里聊了半小时某话题后有人 @ 她问"你觉得呢"，她能接上话。并行话题按触发消息的相关性自然分开，不预分割。
- **话题跟进**：有人 @ 她或**提到她的名字**时进入话题状态；相关消息突发后的停顿点（lull）触发一次自我评估——有增量就主动插话，没有就静默不回应；话题漂移（群聊转场）时权重衰减，掉到阈值后该话题自动结束，她退出不再跟进。她绝不会为了插话而插话：连续两次主动发言没人接，她就闭嘴。
- **统计计数**：谁常说话、谁常 @ 她（落库计数，供后续亲密度功能）。

隐私边界：
- **speaker 伪名化**：库里不存明文 QQ 号——存加盐单向哈希（安装级盐，HMAC-SHA1）；昵称随消息快照仅供摘要可读。防的是消息表被单独拖走时直接读出"谁说了什么"，不防拿到完整数据库+盐的定向攻击。
- **短时缓冲边界**：原文只在 `group_messages` 表存 24 小时，**永不进入长期记忆与日志**——她旁观的群聊只是短时草稿；她参与过的私聊对话才进永久记忆。
- **群号不哈希**（配置白名单已明文）；私聊与非白名单群完全不被观察。
- **关闭方式**：`enabled` 置 `false`，或清空 `observe_groups`（白名单为空时功能自动保持关闭）。
