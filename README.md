...
- **话题会话**：有人 @ 她直接进入话题；**提到她的名字**（无 @）时由杂务模型快速判定是否值得介入（同一话题群每小时最多尝试几次，避免打扰）。介入后每群保持**一个话题会话**，每轮自主决定：说话（简短一句）/ 沉默一拍 / 离场（话题翻页、没人接话、话题沉寂都会让她自然收场，轮次与时长也有硬上限兜底）。会话结束后上下文保留 24 小时供续接，之后自动清理；同群开了新话题，旧上下文立即让位。
