...
| `models` | 模型注册文件、绘图/视觉/杂务（后台苦力任务，可选）配置 |
...
白名单群的只读观察与话题跟进。**总开关与群选择都是运行时状态：在 Web 面板的「观察通道」页勾选即可，写入数据库并立即生效，无需改配置文件、无需重启。**
...
- **话题会话**：有人 @ 她直接进入话题；**提到她的名字**（无 @）时由杂务模型快速判定是否值得介入（同一话题群每小时最多尝试几次，避免打扰）；**群聊突然聊爆**（消息速率超过该群基线——基线按群人数与日常对话量自动学习）且没有进行中的会话时，她会按自己的人格性格自主判断要不要凑个热闹（每天最多尝试 3 次，介入后冷却 2 小时）。介入后每群保持**一个话题会话**，每轮自主决定：说话（简短一句）/ 沉默一拍 / 离场（话题翻页、没人接话、话题沉寂都会让她自然收场，单会话最多陪聊 300 条群消息兜底）。会话结束后上下文保留 2 小时供续接，之后自动清理；同群开了新话题，旧上下文立即让位。
- **杂务模型（可选）**：`models.worker` 从注册表里配一个更便宜的模型名承接判定与上下文压缩（端点与密钥由注册表条目提供）；不配则回落默认模型。
...
- **关闭方式**：面板「观察通道」页关掉总开关（立即生效），或直接改数据库 `perception_meta.feature_enabled=0`。
...
- 模型名类字段（默认模型、记忆提取模型、视觉与杂务端点的模型）从注册表下拉选择，不提供自由输入（画图模型独立于注册表，仍为输入框）；杂务模型不单独成卡，选择框在“对话模型”区；
