...
1. 首次运行：工作目录下没有 `.config.json` 时会自动生成一份**包含全部配置节的骨架**（占位值，密钥为空；Web 面板同时启用，访问令牌打印在启动日志中），也可以手动复制 `config.example.json` 为 `.config.json`（点前缀隐藏文件；从旧版本升级请把原 `config.json` 改名，或用 `--config` 指定旧路径）；
...
| `models` | 模型注册文件、绘图/视觉/轻量模型（后台任务，可选）配置 |
...
白名单群的只读观察与话题跟进。**总开关与群选择都是运行时状态：在 Web 面板的「群聊设置」页勾选即可，写入数据库并立即生效，无需改配置文件、无需重启。**

它做什么：
- **内容短时缓冲**：被观察群的群聊消息（含 Klein 自己的回复）保留**每群最近 300 条 / 24 小时**（双上限，先到先删），供被 @ 时按相关性挑选上下文——群里聊了半小时某话题后有人 @ 她问"你觉得呢"，她能接上话。并行话题按触发消息的相关性自然分开，不预分割。
- **话题会话**：有人 @ 她直接进入话题；**提到她的名字**（无 @）时由轻量模型快速判定是否值得介入（同一话题群每小时最多尝试几次，避免打扰）；**群聊突然聊爆**（消息速率超过该群基线——基线按群人数与日常对话量自动学习）且没有进行中的会话时，她会按自己的人格性格自主判断要不要凑个热闹（每天最多尝试 3 次，介入后冷却 2 小时）。介入后每群保持**一个话题会话**，每轮自主决定：说话（简短一句）/ 沉默一拍 / 离场（话题翻页、没人接话、话题沉寂都会让她自然收场，单会话最多陪聊 300 条群消息兜底）。会话结束后上下文保留 2 小时供续接，之后自动清理；同群开了新话题，旧上下文立即让位。
- **轻量模型（可选）**：`models.worker` 从注册表里配一个更便宜的模型名承接判定与上下文压缩（端点与密钥由注册表条目提供）；不配则回落默认模型。
- **统计计数**：谁常说话、谁常 @ 她（落库计数，供后续亲密度功能）。

隐私边界：
- **短时缓冲边界**：原文与发言人 QQ 号只在 `group_messages` 表存 24 小时（每群 300 条上限），**永不进入长期记忆与日志**——她旁观的群聊只是短时草稿；她参与过的私聊对话才进永久记忆。
- 计数表 `affinity` 存 `(QQ 号, 群号) → 观察/互动次数`；私聊与非观察群完全不被观察。
- **关闭方式**：面板「群聊设置」页关掉总开关（立即生效），或直接改数据库 `perception_meta.feature_enabled=0`。
...
- 侧边栏最顶部的“基础信息”页展示机器人 QQ 头像、昵称、QQ 号、签名（协议端支持时）与 Klein 版本、构建类型、运行时长、通信模式、协议端连接状态；
- 表单化编辑全部配置节（含未知字段），保存时先跑与启动一致的完整校验，不合法直接拒绝并逐字段提示，不会写坏文件；
- 字段标签和分区标题悬停会显示用途说明，包括取值范围与生效方式（保存即生效 / 需重启）；
- “模型供应商”页管理 `source/Model/ModelsName.json`：添加/删除供应商、批量填写模型名称、选择 API 标准，注册表保存后立即热加载，无需重启；
- 模型名类字段（默认模型、记忆提取模型、视觉端点的模型）从注册表下拉选择，不提供自由输入（画图模型独立于注册表，仍为输入框）；轻量模型不单独成卡，选择框在“对话模型”区；
- 密钥字段（`api_key` / `access_token` / `secret`）以掩码显示，浏览器永远拿不到明文；留空表示保持原值，`from_env` 引用原样保留；
- 保存经临时文件原子替换并收紧文件权限为 0600，随后自动刷新内存快照，保存结果以右下角非模态通知弹出：哪些项已生效、哪些项需重启机器人后生效，快照刷新失败也会如实告知。

首次运行没有 `.config.json` 时会自动生成包含全部配置节的骨架配置（占位值，面板中所有模块立即可见），Web 面板随之启用：绑定 `127.0.0.1`、端口 55346，访问令牌随机生成并随每次启动打印在日志中，按日志提示在面板里补全机器人 QQ、默认模型和通信配置后重启即可。
...
可执行文件工作目录下的 `source/`（目录与数据库文件由机器人按需自动创建，首次部署只需放入 `KleinBot.exe` 与 `panel.html`）：

| 目录 | 作用 |
| --- | --- |
| `Model/` | 模型注册文件 `ModelsName.json`（路径固定，见[模型接入](#模型接入)；首次运行时不存在属正常，在面板「模型供应商」页保存后自动创建并加载） |
| `soul.md` | 默认人格文本（原文直接作为 system prompt）；未用 `#设置人格` 设置自定义人格的用户读取此文件，文件缺失时使用内置中文兜底 |
