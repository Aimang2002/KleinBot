...
它统计什么：群成员的**消息条数**（是否 @ 机器人分别计入观察/互动两类）与最后活跃时间（落库 `affinity` 表），以及群内**话题热度**（n-gram 滑窗，仅存内存，重启即失）。这是后续"心境/亲密度"功能的地基，当前版本没有任何对外发言行为。

隐私边界（硬红线）：**任何消息原文永不写入数据库**——落库的只有计数值；话题热度只以 2 字碎片形式短暂存在于内存滑窗（2 小时或最近 500 条即淘汰）。私聊与非白名单群完全不被观察。关闭方式：`enabled` 置 `false`，或清空 `observe_groups`（白名单为空时功能自动保持关闭）。
