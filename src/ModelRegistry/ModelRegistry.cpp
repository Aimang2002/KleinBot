...
std::optional<ChatModel> ModelRegistry::find(const std::string &modelName) const
{
    std::lock_guard<std::mutex> lock(this->mutex);
    for (const auto &chatModel : this->chatModels)
    {
        if (chatModel.modelList.count(modelName) != 0)
        {
            return chatModel;
        }
    }
...
std::vector<ChatModel> ModelRegistry::all() const
{
    std::lock_guard<std::mutex> lock(this->mutex);
...
{
    // 读文件与解析在锁外完成：任何失败路径都不碰旧表（解析失败保留旧表继续服务），
    // 锁内只做一次整体交换，读侧拿到的是完整的新表或完整的旧表
    ...
    for (int i = 0; i < models.size(); i++)
    {
        ...
        parsed.push_back(std::move(cm));
    }

    std::lock_guard<std::mutex> lock(mutex);
    chatModels = std::move(parsed);
    return true;
}
