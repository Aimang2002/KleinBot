...
void Network::reverseWebSocket()
{
	...
	while (true)
	{
		std::string const address = ConfigManager::getInstance().configVariable("SEND_MESSAGE_IP").c_str();
		unsigned short const port = stoi(ConfigManager::getInstance().configVariable("SEND_MESSAGE_PORT"));
