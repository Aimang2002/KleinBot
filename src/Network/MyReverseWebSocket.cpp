...
void MyReverseWebSocket::connectReverseWebSocket()
{
    ...
    while (true)
    {
        std::string const address = ConfigManager::getInstance().configVariable("REVERSEWEBSOCKET_MESSAGE_IP").c_str();
        ...
        acceptor.set_option(boost::asio::ip::tcp::acceptor::reuse_address(true)); // 设置复用
