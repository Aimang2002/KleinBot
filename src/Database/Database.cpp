...
{
	std::ifstream imgIfs(ConfigManager::getInstance().configVariable("FACEURL_PATH"));
	if (!imgIfs.is_open())
	{
...
void ImageURL::writeData()
{
	...
	std::lock_guard<std::mutex> lock(this->img_mutex);
	std::ofstream ofs(ConfigManager::getInstance().configVariable("FACEURL_PATH"), std::ios::app);
	if (!ofs.is_open())
	{
		LOG_ERROR("file open failed!");
		return;
...
{
	std::ifstream ifs(ConfigManager::getInstance().configVariable("ANIMEPICTRUE_PATH"));
	if (!ifs.is_open())
	{
		LOG_ERROR("file open failed!");
		return;
...
{
	std::lock_guard<std::mutex> lock(this->__mutex);
	std::ofstream ofs(ConfigManager::getInstance().configVariable("ANIMEPICTRUE_PATH"), std::ios::app);
	if (!ofs.is_open())
	{
		LOG_ERROR("file open failed!");
		return;
...
{
	std::ifstream ifs(ConfigManager::getInstance().configVariable("CNGPICTRUE_PATH"));
	std::pair<int, std::string> p;
	char buff = ' ';
	if (!ifs.is_open())
	{
