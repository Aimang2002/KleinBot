...
PerceptionStore::PerceptionStore(const std::string &dbPath)
{
    ...
    const char *ddl =
        ...
        " PRIMARY KEY (speaker_id, group_id));";
    char *err = nullptr;
...
void PerceptionStore::upsertBatch(const std::vector<AffinityDelta> &deltas)
{
    ...
    const char *sql =
        "INSERT INTO affinity (speaker_id, group_id, observed_count, interaction_count, last_seen_ts)"
        " VALUES (?1, ?2, ?3, ?4, ?5)"
        " ON CONFLICT(speaker_id, group_id) DO UPDATE SET"
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &statement, nullptr) == SQLITE_OK)
    {
        ...
        {
            const std::string speaker = perception::speakerIdOf(salt, delta.userId);
            sqlite3_reset(statement);
            sqlite3_clear_bindings(statement);
            sqlite3_bind_text(statement, 1, speaker.c_str(), -1, SQLITE_TRANSIENT);
